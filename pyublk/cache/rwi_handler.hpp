...
inline int RWIHandler::submit(std::shared_ptr<read_query> rq) noexcept {
  assert(rq);
...
inline int RWIHandler::submit(std::shared_ptr<write_query> wq) noexcept {
  assert(wq);

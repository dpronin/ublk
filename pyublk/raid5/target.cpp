...
ssize_t Target::write(std::span<std::byte const> buf,
  ...
  auto const stripe_data_sz = cached_stripe_view().size_bytes() - strip_sz_;

  auto stripe_id{offset / stripe_data_sz};
  auto stripe_offset{offset % stripe_data_sz};
  ...
  while (!buf.empty()) {
    ...
    auto const chunk{
        buf.subspan(0, std::min(stripe_data_sz - stripe_offset, buf.size()))};

    /* Split data strips in two parts of u64 chunks*/
    auto const data_fp = to_span_of<uint64_t>(
        cached_stripe_view().subspan(0, parity_stripe_offset));
    ...
    if (auto const res = read_data_skip_parity(stripe_id * (hs_.size() - 1), 0,
                                               std::as_writable_bytes(data_fp));
        res < 0) [[unlikely]] {
      return res;
    }

    if (auto const res = read_data_skip_parity(
            stripe_id * (hs_.size() - 1) + data_fp.size_bytes() / strip_sz_, 0,
            std::as_writable_bytes(data_lp));
        res < 0) [[unlikely]] {
      return res;
    }
    ...
    algo::copy(to_span_of<std::byte const>(chunk_fp),
               std::as_writable_bytes(data_fp).subspan(data_fp_offset,
                                                       chunk_fp.size()));
    ...
    algo::copy(to_span_of<std::byte const>(chunk_lp),
               std::as_writable_bytes(data_lp).subspan(data_lp_offset,
                                                       chunk_lp.size()));

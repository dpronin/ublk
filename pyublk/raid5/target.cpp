...
class r5 final : public raidsp::Target {
public:
...
struct transition_table {
  auto operator()() noexcept {
    ...
    return make_transition_table(
       // online state
       *"online"_s + event<erq> [ ([](erq const &e, r5& r){ e.r = r.process(std::move(e.rq)); return 0 == e.r; }) ]
      , "online"_s + event<erq> = "offline"_s
      , "online"_s + event<ewq> [ ([](ewq const &e, r5& r){ e.r = r.process(std::move(e.wq)); return 0 == e.r; }) ]
      , "online"_s + event<ewq> = "offline"_s
      , "online"_s + event<estripecohcheck> / [](estripecohcheck const &e, r5 const& r) { e.r = r.is_stripe_parity_coherent(e.stripe_id); }
       // offline state
...
public:
  explicit impl(uint64_t strip_sz, std::vector<std::shared_ptr<IRWHandler>> hs)
      : r5_(strip_sz, std::move(hs)), fsm_(r5_) {}

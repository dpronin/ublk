...
constexpr auto for_each_celld =
    []<typename T>(std::integral auto celldn, std::integral auto cellds_nr,
                   std::span<ublk_celld const> cellds, std::span<T> cells,
                   auto &&f, auto &&...args) {
      if (auto cellds_left = cellds_nr;
          cellds_left > 0 && celldn < cellds.size()) [[likely]] {

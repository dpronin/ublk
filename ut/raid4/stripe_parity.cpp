...
class RAID4_StripeParity : public Test {
protected:
  ...
  void SetUp() override {
    ...
    target_ = std::make_unique<ublk::raid4::Target>(kStripSz, hs_);
  }
  ...
  std::unique_ptr<ublk::raid4::Target> target_;
};

} // namespace

TEST_F(RAID4_StripeParity, AllStripesParityIncoherentAtInit) {
  for (auto stripe_id : std::views::iota(0uz, kStripesNr))
    EXPECT_FALSE(target_->is_stripe_parity_coherent(stripe_id));
}

TEST_F(RAID4_StripeParity, StripeParityBecomesCoherentAtEachFullStripeWrite) {
  auto buf{
